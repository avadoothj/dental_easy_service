...
export default function SearchFilterMobile() {
	...
	const roleTypes = [{ id: "all", label: "All" }, ...roleTypesList];
	...
	if (searchParams.get("type")) {
		const temp = roleTypes.filter((x) => x.id == searchParams.get("type"));
		if (temp.length > 0) {
			defaultSelected.type = temp[0].id;
		}
	}
	...
	const [type, settype] = useState(defaultSelected.type);
					...
					<div className={style.searchboxmob}>
						{keyword.length == 0 && (
							<button
								...
								onClick={() => {
									jQuery("#role_search_mob").trigger("focus");
								}}
							>
								<svg
