...
export default function SelectMultiSearch(props) {
  ...
  const processDataList = () => {
    ...
    if (options.multiple) {
      ...
      const selectedObj = mainData.find(
        (x) => x.id == (options.defaultSelected[0] ?? -1),
      );
  ...
  const handleSelect = (event, newValue) => {
    ...
    if (clickedId === "all") {
      ...
      const selectedIds = options.multiple
        ? newValue.map((x) => x.id)
        : [newValue?.id];
      ...
      if (allItem) {
        ...
        allItem.checked =
          nonAllOptions.length > 0 && nonAllOptions.every((x) => x.checked);
      }
    }
    ...
    <>
      ...
      <Autocomplete
        ...
        renderOption={(props, option, { selected }) => {
          ...
          return (
            <li {...optionProps} key={key} value={option.id}>
        ...
        renderInput={(params) => (
          <TextField {...params} placeholder={options.placeholder} />
        )}
      />
    </>
  );
