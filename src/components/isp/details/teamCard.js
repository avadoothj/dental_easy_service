...
import { editUserStatus } from "@/controllers/operators";
import { AppContext } from "@/contextProvider";
import messages from "@/utils/messages";
import { getTokenForLoginAsUser } from "@/controllers/profile";

export default function TeamCard({ item, isp, handleTeamSelect, reloadData }) {
	const { showAlert, user } = useContext(AppContext);

	let activeStatus = "Active";
	const userAction = item.user_block == 1 ? 1 : 0;
			...
			>
					...
					item.permanent_block != 1 && (
						<div className="threedotpop">
							<Dropdown>
								...
								<Dropdown.Menu>
									<Dropdown.Item onClick={(e) => handleTeamSelect(item)}>
										Edit
									</Dropdown.Item>
				...
				<div className={style.tname}>
					<SimpleTooltip text={item.name}>
						<div className={style.tnamelft}>{item.name}</div>
					</SimpleTooltip>
					{user?.allowedLinks.indexOf("/stakeholderAddEdit") >= 0 &&
						item.permanent_block != 1 && (
				...
				<div className={style.topname}>
					<div className={style.topnamesec}>
						<p className={style.tptxt}>ISP</p>
						<p className={style.btmtxt}>
							<SimpleTooltip text={isp.oper_name}>
				...
				<div className={style.btmname}>
					<div className={style.topnamesec}>
						<p className={style.tptxt}>Added on</p>
						<p className={style.btmtxt}>{formatDate(item.inserted_date)}</p>
					</div>
					<div className={style.topnamesec}>
