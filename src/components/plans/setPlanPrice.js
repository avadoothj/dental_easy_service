...
export default function SetPlanPrice({ planDetail, handleClose, reloadData, operatorList }) {
	const {
		register,
		handleSubmit,
		setValue,
	...
	const priceSetFor = user?.user_type == "isp" ? "Operator(s)" : "Subscriber";
	...
	const handleFormSubmit = async () => {
		...
		setIsLoading(true);
		const response = await addPlanRate(formData);
		setIsLoading(false);
		...
		<>
			...
			<form onSubmit={handleSubmit(handleFormSubmit)}>
				<ul className="setsubmid">
					...
					{user?.user_type == "isp" && (
						<li id="r">
							<div className="setsubl mt-10">Operator(s)</div>
